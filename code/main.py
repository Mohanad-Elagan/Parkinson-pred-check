...
with gr.Blocks(css="#img0, #img1 {background:#0B0F19}") as app:
    ...
    with gr.Row() as row:
        with gr.Column():
            img1 = gr.Image('../data/brain.svg', show_label=False, visible=False)
        with gr.Column():
            img1 = gr.Image('../data/brain.svg', show_label=False, elem_id="img0", invert_colors=False).style(full_width=True, height=135)
        with gr.Column():
            img1 = gr.Image('../data/brain.svg', show_label=False, visible=False)
    with gr.Row() as row:
        with gr.Column():
